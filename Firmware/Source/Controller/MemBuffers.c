// Header
#include "MemBuffers.h"

// Includes


// Variables
//
Int16U MEMBUF_EP_Vd[VALUES_x_SIZE] = {0};
Int16U MEMBUF_EP_Id[VALUES_x_SIZE] = {0};
Int16U MEMBUF_EP_Vg[VALUES_x_SIZE] = {0};
Int16U MEMBUF_EP_Ig[VALUES_x_SIZE] = {0};

volatile Int32U MEMBUF_DMA[VALUES_POWER_DMA_SIZE] = {0};
volatile Int16U MEMBUF_DMA_Vd[VALUES_POWER_DMA_SIZE] = {0};
volatile Int16U MEMBUF_DMA_Id[VALUES_POWER_DMA_SIZE] = {0};
volatile Int16U MEMBUF_DMA_Vg[VALUES_GATE_DMA_SIZE] = {0};
volatile Int16U MEMBUF_DMA_Ig[VALUES_GATE_DMA_SIZE] = {0};
